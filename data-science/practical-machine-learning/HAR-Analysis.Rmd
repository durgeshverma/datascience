...
Training Data file has tabular data that contains `r nrow(rawdata)` rows and `r ncol(rawdata)`. 
...
predict_table = data.frame(model_rf_pca_vals=predict(rf.pca$finalModel,test.pca),model_rf_cs_vals=predict(rf.cs$finalModel,test.cs))
predict_table = mutate(predict_table,match=ifelse(model_rf_pca_vals==model_rf_cs_vals,TRUE,FALSE))
qplot(model_rf_pca_vals,model_rf_cs_vals,data=predict_table,colour=match) + ggtitle("Model Prediction Comparison")
```
